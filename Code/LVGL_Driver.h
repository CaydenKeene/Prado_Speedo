...
// Last gesture seen by the touch read callback, latched so app code can poll it
// at its own rate (the callback otherwise clears the gesture every read).
// Consumer should set it back to NONE after handling.
extern volatile GESTURE Lvgl_Last_Gesture;
